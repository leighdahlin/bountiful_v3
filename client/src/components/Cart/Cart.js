...
const Cart = () => {
  ...
  function calculateTotal() {
    ...
    state.cart.forEach((item) => {
      sum += item.item_price * item.purchaseQuantity;
    });
  ...
  if (!state.cartOpen) {
    return (
      <div className="cart-closed" onClick={toggleCart}>
        <span id="trash" role="img" aria-label="trash" >
          🛒
        </span>
      </div>
    );
      ...
      {state.cart.length ? (
        <div>
          ...
          <div className="flex-row space-between">
            ...
            {Auth.loggedIn() ? (
              <button className="btn" id="checkout" onClick={submitCheckout}>Checkout</button>
            ) : (
